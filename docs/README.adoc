...
| xref:websocket.adoc[WebSocket Bridge]
| Real-time web client access via a WebSocket-to-IRC relay on port 6680.
...
| WebSocket Bridge
| 6680
| `ws://<host>:6680/irc` — web client IRC access
...
| `IRC_WS_ENABLED`
| `true`
| Set to `false` to disable the WebSocket bridge

| `IRC_WS_PORT`
| `6680`
| WebSocket bridge listen port

| `IRC_WS_HOST`
| `0.0.0.0`
| Network interface the WebSocket bridge binds to

| `IRC_HOST`
| `127.0.0.1`
| IRC server host the bridge connects to internally
