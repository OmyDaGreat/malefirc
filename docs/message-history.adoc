...
All history queries are performed via `MessageHistoryService`.  Callers are responsible
for enforcing the following access-control rules before returning data to a requesting user:

[#_access_control_rules]
=== Rules

. Only authenticated users should be permitted to query history.
. Private message history (`getPrivateHistory`) must be restricted to the two participants.
. `getMessagesBySender` should only allow users to retrieve their own sent messages.
. Results must be filtered to exclude messages from users who have disabled `allow_history_access`
  (see xref:security.adoc#_user_privacy_settings[Privacy Settings]).

These rules are checked against the `AccountTable.allowHistoryAccess` column.

=== Data Shape

`MessageHistoryService` returns `MessageHistoryData` objects:
...
data class MessageHistoryData(
    ...
    val sender: String,        // IRC nickname
    val target: String,        // #channel or recipient nick
    val message: String,
    val messageType: String,   // "PRIVMSG" or "NOTICE"
    val isChannelMessage: Boolean,
    val replyToId: Long?,      // parent message ID for threaded replies
)
----
