...
class IRCServer(
    ...
    private suspend fun handleMessage(connection: ClientConnection, message: IRCMessage) {
        val user = connection.user
        
        when (message.command) {
            IRCCommand.NICK -> handleNick(connection, message)
            IRCCommand.USER -> handleUser(connection, message)
            IRCCommand.JOIN -> handleJoin(connection, message)
            IRCCommand.PART -> handlePart(connection, message)
            IRCCommand.PRIVMSG -> handlePrivmsg(connection, message)
            IRCCommand.QUIT -> handleQuit(connection, message)
            IRCCommand.PING -> handlePing(connection, message)
            IRCCommand.TOPIC -> handleTopic(connection, message)
            IRCCommand.NAMES -> handleNames(connection, message)
            IRCCommand.LIST -> handleList(connection, message)
            IRCCommand.WHO -> handleWho(connection, message)
            else -> {
                if (user.isRegistered()) {
                    sendMessage(connection, IRCMessageBuilder.numericError(
                        serverName, user.nickname!!, IRCReply.ERR_UNKNOWNCOMMAND,
    ...
    private suspend fun handleNick(connection: ClientConnection, message: IRCMessage) {
        val user = connection.user
        val newNick = message.params.firstOrNull()
        
        if (newNick.isNullOrBlank()) {
            sendMessage(connection, IRCMessageBuilder.numericError(
                serverName, user.nickname ?: "*", IRCReply.ERR_NONICKNAMEGIVEN,
                "No nickname given"
            ))
            return
        }
        ...
        if (users.containsKey(newNick)) {
            sendMessage(connection, IRCMessageBuilder.numericError(
                serverName, user.nickname ?: "*", IRCReply.ERR_NICKNAMEINUSE,
    ...
    private suspend fun handleUser(connection: ClientConnection, message: IRCMessage) {
        val user = connection.user
        
        if (user.username != null) {
            sendMessage(connection, IRCMessageBuilder.numericError(
                serverName, user.nickname ?: "*", IRCReply.ERR_ALREADYREGISTERED,
        ...
        if (message.params.size < 3) {
            sendMessage(connection, IRCMessageBuilder.numericError(
                serverName, user.nickname ?: "*", IRCReply.ERR_NEEDMOREPARAMS,
    ...
    private suspend fun handleJoin(connection: ClientConnection, message: IRCMessage) {
        val user = connection.user
    ...
    private suspend fun handlePart(connection: ClientConnection, message: IRCMessage) {
        val user = connection.user
        if (!user.isRegistered()) return
        ...
        if (!channel.users.containsKey(user.nickname)) {
            sendMessage(connection, IRCMessageBuilder.numericError(
                serverName, user.nickname!!, IRCReply.ERR_NOTONCHANNEL,
    ...
    private suspend fun handlePrivmsg(connection: ClientConnection, message: IRCMessage) {
        val user = connection.user
        if (!user.isRegistered()) return
        ...
        if (target.startsWith('#')) {
            ...
            if (channel == null || !channel.users.containsKey(user.nickname)) {
                sendMessage(connection, IRCMessageBuilder.numericError(
                    serverName, user.nickname!!, IRCReply.ERR_CANNOTSENDTOCHAN,
            ...
            if (targetConn == null) {
                sendMessage(connection, IRCMessageBuilder.numericError(
                    serverName, user.nickname!!, IRCReply.ERR_NOSUCHNICK,
    ...
    private suspend fun handleQuit(connection: ClientConnection, message: IRCMessage) {
        disconnect(connection, message.trailing ?: "Client quit")
    }
    
    private suspend fun handlePing(connection: ClientConnection, message: IRCMessage) {
        val server = message.params.firstOrNull() ?: serverName
        sendMessage(connection, IRCMessageBuilder.pong(serverName, server))
    }
    
    private suspend fun handleTopic(connection: ClientConnection, message: IRCMessage) {
        val user = connection.user
        if (!user.isRegistered()) return
        ...
        if (channel == null) {
            sendMessage(connection, IRCMessageBuilder.numericError(
                serverName, user.nickname!!, IRCReply.ERR_NOSUCHCHANNEL,
    ...
    private suspend fun handleNames(connection: ClientConnection, message: IRCMessage) {
        val user = connection.user
        if (!user.isRegistered()) return
    ...
    private suspend fun handleList(connection: ClientConnection, message: IRCMessage) {
        val user = connection.user
        if (!user.isRegistered()) return
        
        sendMessage(connection, IRCMessage(
            prefix = serverName,
    ...
    private suspend fun handleWho(connection: ClientConnection, message: IRCMessage) {
    ...
    private suspend fun disconnect(connection: ClientConnection, reason: String = "Connection closed") {
        val user = connection.user
    ...
    private suspend fun sendMessage(connection: ClientConnection, message: IRCMessage) {
        try {
    ...
    private suspend fun broadcastToChannel(channel: IRCChannel, message: IRCMessage, except: String? = null) {
