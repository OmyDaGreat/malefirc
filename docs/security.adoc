...
* [ ] Enabled SSL/TLS (see xref:tls.adoc[TLS / SSL Support])
...
| `IRC_TLS_ENABLED` | `false` | Enable dedicated TLS listener (port 6697)
| `IRC_TLS_PORT` | `6697` | Port for dedicated TLS connections
| `IRC_TLS_CERT_FILE` | — | Path to PEM certificate file (use with `IRC_TLS_KEY_FILE`)
| `IRC_TLS_KEY_FILE` | — | Path to PEM private key file (use with `IRC_TLS_CERT_FILE`)
| `IRC_TLS_KEYSTORE_PATH` | — | Path to JKS or PKCS12 keystore (auto-generated if absent)
| `IRC_TLS_KEYSTORE_PASSWORD` | `changeit` | Keystore password
| `IRC_TLS_KEY_PASSWORD` | `changeit` | Private key password (JKS/PKCS12 only)
| `IRC_TLS_KEY_ALIAS` | `irc` | Key alias in keystore
|===
...
* The `AUTHENTICATE PLAIN` exchange transmits credentials over the IRC connection — always use SSL/TLS in production (see xref:tls.adoc[TLS / SSL Support]).
...
* Be aware that messages are transmitted in plaintext unless SSL/TLS is enabled (see xref:tls.adoc[TLS / SSL Support]).
